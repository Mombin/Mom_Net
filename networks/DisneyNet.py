...
class Encoder_(nn.Module):
    def __init__(self, cfg, n_label=10):
        super().__init__()
        ...
        self.linear1 = nn.Linear(512, n_label) #init n_label : 1
        self.linear2 = nn.Linear(n_label, 512)
...
class Decoder_(nn.Module):
    def __init__(self, cfg, code_dim=512 - 10, n_label=10):
        super().__init__()
    ...
    def forward(self, input, step=0, alpha=-1):
        ...
        out = torch.unsqueeze(input, 2).unsqueeze(3)
...
class autoencoder(nn.Module):
    def __init__(self, cfg, netE, netD):
        ...
        self.enc = netE
        self.dec = netD
        #self.enc = Encoder_(cfg)
        #self.dec = Decoder_(cfg)


    def forward(self, x, step, alpha):

        x = self.enc(x, step, alpha)
        x = self.dec(x, step, alpha)
        return x
