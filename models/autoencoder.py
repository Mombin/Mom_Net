...
class encoder_(nn.Module):
    def __init__(self, cfg):
        ...
        nn.LeakyReLU(0.2, inplace=True),
        #1024*8*8
        nn.Conv2d(self.nef*8, self.nef*16, kernel_size=4, stride=2, padding=1, bias=False),
        nn.BatchNorm2d(self.nef*16),
        nn.LeakyReLU(0.2, inplace=True),
        #2048*4*4
        nn.Flatten(),
...
class decoder_(nn.Module):
    def __init__(self, cfg):
        ...
        nn.ConvTranspose2d(self.nef*16, self.nef*8, kernel_size=4, stride=2, padding=1, bias=False),
        nn.BatchNorm2d(self.nef*8),
        nn.LeakyReLU(0.1, inplace=True),
        nn.ConvTranspose2d(self.nef*8, self.nef*4, kernel_size=4, stride=2, padding=1, bias=False),
        nn.BatchNorm2d(self.nef*4),
        nn.LeakyReLU(0.1, inplace=True),
